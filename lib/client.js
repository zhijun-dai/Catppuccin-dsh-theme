      ...
      "--shiki-token-changed": "#fab387"
		...
		function apply(ctx) {
			...
			const SURFACE_RULES = [
				...
				// NOTE: no backdrop-filter here on purpose. The panes sit on a
				// solid frame (nothing behind them to blur), and
				// backdrop-filter turns the element into the containing
				// block for fixed-position descendants — the settings
				// overlay/poppers would misplace against the pane instead of
				// the viewport. Translucent fill + rim + edge light + drop
				// still reads as a glass card against the solid ground.
				[
