    ...
    "tokens": {
      ...
      "--shiki-token-changed": "#fab387"
		...
		const zh = {
			...
			"skin.glassHint": "磨砂玻璃卡片，模糊度、磨砂度与背景亮度可调",
			"skin.glassEnabled": "已开启",
			"skin.glassDisabled": "已关闭",
		...
		const en = {
			...
			"skin.glassHint": "Frosted glass cards — blur, frost and brightness are adjustable",
			"skin.glassEnabled": "On",
			"skin.glassDisabled": "Off",
...
`;
		...
		/**
		 * Glassmorphism control panel: master switch, mica/compat mode, the
		 * three presets and three knobs (blur radius, frost opacity,
		 * background brightness — the last one halves its range on the dark
		 * scheme). Changes apply live; the module-level scheduleFlush
		 * debounces the host-state PUT.
		 */
		function GlassRow({ t }) {
			...
			const preset = GLASS_PRESETS.find((candidate) => candidate.blur === state.blur && candidate.frost === state.frost && candidate.brightness === state.brightness) ?? null;
			const modeChip = (mode, label) => (0, react_jsx_runtime.jsx)("button", {
				type: "button",
				disabled: !on,
				onClick: () => update({ mode }),
				...
				style: {
					...styles.glassChip,
					...(state.mode === mode ? styles.glassChipActive : {}),
					...(!on ? styles.glassChipDisabled : {})
				},
				children: label
			});
			const sliderRow = (label, min, max, step, value, setter) => (0, react_jsx_runtime.jsxs)("div", {
				style: styles.glassSubRow,
				children: [
					(0, react_jsx_runtime.jsx)("span", {
						style: styles.glassLabel,
						children: label
					}),
				...
				children: [
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsx)("button", {
								...
								style: {
									...styles.glassChip,
									...(on ? styles.glassChipActive : {})
								},
								children: t(on ? "skin.glassEnabled" : "skin.glassDisabled")
							}),
							(0, react_jsx_runtime.jsx)("span", {
								style: styles.updateDetail,
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							GLASS_PRESETS.map((candidate) => (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								disabled: !on,
								onClick: () => update({ blur: candidate.blur, frost: candidate.frost, brightness: candidate.brightness }),
								"aria-pressed": preset !== null && preset.id === candidate.id,
								style: {
									...styles.glassChip,
									...(preset !== null && preset.id === candidate.id ? styles.glassChipActive : {}),
									...(!on ? styles.glassChipDisabled : {})
								},
								children: t(`skin.glassPreset${candidate.id.charAt(0).toUpperCase()}${candidate.id.slice(1)}`)
							}, candidate.id))
						]
					}),
					(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							modeChip("mica", t("skin.glassModeMica")),
							modeChip("compat", t("skin.glassModeCompat"))
						]
					}),
					sliderRow(t("skin.glassBlur"), 0, 40, 0.5, state.blur, (blur) => update({ blur })),
					sliderRow(t("skin.glassFrost"), 0, 100, 1, state.frost, (frost) => update({ frost })),
					sliderRow(t("skin.glassBrightness"), dark ? 0 : 50, dark ? 50 : 100, 1, state.brightness, (brightness) => update({ brightness }))
				]
			});
		}
