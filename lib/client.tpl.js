...
`;
		...
		//#region dsh-catppuccin: settings row subscription
		/**
		 * Skin row subscription: the row reads the live theme preference
		 * through react.useSyncExternalStore, so the plugin only owns a
		 * listener set it pokes on theme/change. Since DSH 0.1.2 the frozen
		 * module table has no dsh-client-runtime (its slots role moved to
		 * dsh-client-ui-renderer), so nothing in this bundle may require()
		 * that package — snapshot stores are plugin-owned here.
		 */
		 ...
		 */
		function SkinRow({ t, setSkin, getSkin, subscribeSkin }) {
			const skin = react.useSyncExternalStore(subscribeSkin, getSkin);
		...
		function apply(ctx) {
			...
			const skinInjected = () => ({
				...
				getSkin: () => ctx.theme.getTheme().preference,
				subscribeSkin: (listener) => {
